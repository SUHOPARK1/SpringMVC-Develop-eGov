...
$.getJSON(`${x.ctx}/teachers/page/${x.pageSize}/${x.pageNum}/subject/${x.subNum}/${x.examDate}`, d => {
      ...
      if(page.existPrev){
         ...
         .click(e=>{
            ...
            tea.grade_mgmt({ctx: x.ctx, pageSize: `10`, pageNum: page.prevBlock, subNum: list[0].subNum, examDate: list[0].examDate})
         })
      }
      $.each(
          ...
          (i, j) => {
            $(`<a/>`)
               ...
               .click(e=>{
                  e.preventDefault()
                  $(`#mgr-data-mgt-stu`).empty()
                  tea.grade_mgmt({ctx: x.ctx, pageSize: `10`, pageNum: j,subNum: list[0].subNum, examDate: list[0].examDate})
               })
      })
      if(page.existNext){
         $(`<a/>`)
         ...
         .click(e=>{
            e.preventDefault()
            $(`#mgr-data-mgt-stu`).empty()
            tea.grade_mgmt({ctx: x.ctx, pageSize: `10`, pageNum: page.nextBlock, subNum: list[0].subNum, examDate: list[0].examDate})
         })
      }
